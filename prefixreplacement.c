...
typedef struct _FizzBuzzTemplate {
    ...
    char template[668*207];  // Near UINT64_MAX digit length is 20 and one iteration uses 207 characters. 
} FizzBuzzTemplate;

static void FizzBuzzTemplate_initialize(FizzBuzzTemplate *fizzbuzz_template, uint64_t start) 
{
    if (start % 10000 != 0) {
        fprintf(
            stderr,
    ...
    fizzbuzz_template->template_size = fizzbuzz_memoized_unrolled(
        start, start + 10000, fizzbuzz_template->template);
}

static size_t FizzBuzzTemplate_apply_by4(
    FizzBuzzTemplate *restrict fizzbuzz_template,
    uint64_t start, 
    char *restrict buffer
)
{   
    /* memcpy is very optimized to handle such large chunks. First copy 
       template to buffer.  */
    memcpy(buffer, fizzbuzz_template->template, fizzbuzz_template->template_size);
